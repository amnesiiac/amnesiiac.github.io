...
### # background: why we need this script?
sometimes, the atime/mtime of files in git repo is changed by some process,
...
### # code in action: sequential get & modify
```text
#!/bin/sh

projpath='/repo/reborn/sharedhome/aports'

set_file_time_as_latestcommit() {
    for file in `find "${projpath}" -type f -not -path "${projpath}/.git/*"`; do
        echo "setting time for $file"
        # get commitid -> get commit related time -> modify mtime for the file using the derived 'lastest commit time'
        git rev-list -n 1 HEAD $file | xargs git show -s --format=%ai \
            | awk -v file=$file '{system("touch -d \"" $1 " " $2 "\" " file)}';
    done
}
...
```
the above script is using a sequential way to change each file's mtime to latest commit time,
so the performance is very low. the alpine/aport repo, contains approximately 10000+ files to change,
will comsume more than 17min to rectify all the file timestamp.

<hr>

### # code in action: efficient parallel method by python script (todo)
```text
#!/usr/bin/env python3
#
...
apply it for the rebornlinux/aport repo (10000+files), the statistics are as follows:
```text
$(/repo/reborn/sharedhome/aports) sudo python3 time.py
18,787 files to be processed in work dir
Statistics:
         0.96 seconds
       18,949 log lines processed
           50 commits evaluated
       10,302 directories updated
       18,787 files updated
```
