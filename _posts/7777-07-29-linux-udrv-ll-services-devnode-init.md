...
static int udrv_devnode_load_all_jsons(char* json_pathname){
    ...
    ret = udrv_path_dirname(json_pathname, &json_dir);           // get jsonfile dir for loading sub json files (utils.c)
    if(ret){
        pr_error("get json file dir failed, error: %d\n", ret);
        return ret;
    }
...
static void udrv_devnode_load_subjsons(char* json_dir, devnode_t* node){
    ...
    devnode_object_foreach_safe(node, tmp1, key1, value1){       // for each k,v in json node
        if(!udrv_is_import_key(key1)){                           // (1) not import key, recursively load the sub node
            udrv_devnode_load_subjsons(json_dir, value1);
            continue;
        }
        json_filename = (char*) json_string_value(value1);       // (2) import key, buildup the subjson import path
        ...
        value2 = udrv_devnode_load_json(json_pathname);          // (2.1) load the imported json data to devnode
        udrv_str_free(json_pathname);
        if(!value2){
            continue;
        }
        udrv_devnode_load_subjsons(json_dir, value2);            // (2.2) recursively load the import node's sub node
...
static int udrv_devnode_check_duplicated_devname(){
    ...
    devnode_object_foreach(json_root, level_1_node_name, level_1_node{   // ret number of l1 + l2 devices
        if(!udrv_devnode_has_compatible(level_1_node)){                  // json node has no compatible field -> skip
            continue;
        }
